...
void print_collisions(int* collisions, double expected_value, int capacity, int num_lines)
{
    double x = 0;
    int slots_filled = 0;
    int counted = 0;
    for ( int i=1; i < capacity; ++i )
    {
        ...
        counted += count * i;
        slots_filled += count;
        x += (((i - expected_value) * (i - expected_value))/expected_value) * count;
        if (counted == num_lines) break;
    }
    x += expected_value * (capacity - slots_filled);
...
int main( int argc, char** argv ) {
    ...
    double expected_value = ((double)num_lines)/((double)capacity);
    std::cout << "Expected Value: " << expected_value << std::endl;
    std::cout << "Capacity: " << capacity << std::endl;
    std::cout << "Super Fast Hash" << std::endl;
    print_collisions(collisions, expected_value, capacity, num_lines);
    delete[] collisions;
    std::cout << "Book Hash" << std::endl;
    print_collisions(collisions2, expected_value, capacity, num_lines);
    delete[] collisions2;
    std::cout << "Cat Hash" << std::endl;
    print_collisions(collisions3, expected_value, capacity, num_lines);
    std::cout << "My Hash" << std::endl;
    print_collisions(collisions4, expected_value, capacity, num_lines);
    delete[] collisions4;
    return 0;
}